...
import { selectCartModule, selectProductAmount } from '@/app/redux/features/cart/selector';
import { cartActions, cartItemsActions, totalActions } from '@/app/redux/features/cart';
...
export const TicketCardCounter: FC<TicketCardCounter> = ({...countProps}) => {
    ...
    // console.log(useSelector((state) => selectCartModule(state)))
    // console.log(id)
    // console.log(amount);
        ...
        <>
                ...
                <Button variant='plus' disabled={isPlusDisabled} onClick={(e: MouseEvent) => {
                    ...
                    if (!isMemorized) {
                        dispatch(cartItemsActions.add({
                            id, genre: genre, title, posterUrl
                        }));
                        setIsMemorized(true);
                    }
                } }></Button>
            ...
            {!!isPopupOpen && createPortal(
                <><BackgroundBlur onClick={close} />
                    <Popup id={id} onClick={handleYesButton} close={close} ref={popupElement} />
                </>,
                document.body
            )
            }
...
export const TicketCard: FC<TicketCardProps> = ({...props}) => {
    ...
    return ( // еще значит вверх надо передавать
        <div className={styles.ticket_card}>
            ...
            <div className={styles.ticket_card__counter}>
                <TicketCardCounter id={id} cart={cart || false} genre={genreR || genre} title={title} posterUrl={posterUrl} /> 
            </div>
        </div>
    )
}
